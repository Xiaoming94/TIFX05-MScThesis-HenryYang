...
def vote_predict(preds,ensemble_size):
    votes = 0
    for p in preds:
...
def ensemble_measures(member_predictions, labels):
    ...
    member_predictions = vote_predict(member_predictions, ensemble_size)
    c_error = calc_cerror(member_predictions, labels)
    bits = calc_entropy(member_predictions)
...
#
        ...
        filename1 = 'vote_mnist_results_100-trial%s' % (t + 1)
        filename2 = 'vote_digits_results_100-trial%s' % (t + 1)
